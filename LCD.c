...
static uint8_t t_bar[2] = {0x00,0x00};
...
void LCD_PIN_Init(void){
	...
	//Enable HSI to Ports A, B, C, and D
	while((RCC->CR & RCC_CR_HSIRDY) == 0){RCC->CR |= RCC_CR_HSION;}
	if((RCC->AHB2ENR & RCC_AHB2ENR_GPIOAEN) == 0){RCC->AHB2ENR |= RCC_AHB2ENR_GPIOAEN;}
	if((RCC->AHB2ENR & RCC_AHB2ENR_GPIOBEN) == 0){RCC->AHB2ENR |= RCC_AHB2ENR_GPIOBEN;}
	if((RCC->AHB2ENR & RCC_AHB2ENR_GPIOCEN) == 0){RCC->AHB2ENR |= RCC_AHB2ENR_GPIOCEN;}	
	if((RCC->AHB2ENR & RCC_AHB2ENR_GPIODEN) == 0){RCC->AHB2ENR |= RCC_AHB2ENR_GPIODEN;}
	//Configure PA6-10 & 15 as AF (11)
	GPIOA->MODER &=  (0x3FC00FFF); 
	GPIOA->MODER |=  (0x802AA000);
	GPIOA->AFR[0] &= (0x00FFFFFF);
	GPIOA->AFR[0] |= (0xBB000000);GPIOA->AFR[1] &= (0x0FFFF000);
	GPIOA->AFR[1] |= (0xB0000BBB);
	//Configure PB0,1,4,5,9,12,13,14,15 as AF (11)
	...
	//Configure PC3,4,5,6,7,8 as AF (11)
...
void LCD_Configure(void){
	//Configure BIAS bits of LCD CR to bias = 1/3
	LCD->CR &= ~LCD_CR_BIAS_0;
	LCD->CR |= LCD_CR_BIAS_1;
	//Configure DUTY of LCD_CR to duty = 1/4
	LCD->CR &= ~LCD_CR_DUTY_2;
	LCD->CR |= (LCD_CR_DUTY_1 | LCD_CR_DUTY_0);
	//Set contrast of LCD_FCR to max value 111 (CC bits)
	LCD->FCR |= (LCD_FCR_CC_0 | LCD_FCR_CC_1 | LCD_FCR_CC_2);
	//Configure PON bits of LCD_FCR and set the pulse on period to 111
	LCD->FCR |= (LCD_FCR_PON_0 | LCD_FCR_PON_1 | LCD_FCR_PON_2);
	//Disable MUX_SEG of LCD_CR
	...
	//Enable the LCD by setting the LCDEN bit of LCD_CR
	LCD->CR |= LCD_CR_LCDEN;
	//Wait until the LCD is enabled
	while((LCD->SR & LCD_SR_ENS)==0);
	//Wait until the LCDbooster is ready
	while((LCD->SR & LCD_SR_RDY)==0);
	//enable display request is LCD_SR
	LCD->SR |= LCD_SR_UDR;
}
...
void LCD_Clear(void){
	...
	// Clear display
	LCD->RAM[0] &= 0x083D0C87;
	LCD->RAM[1] &= 0xFFFFFFF0;
	LCD->RAM[2] &= 0x083D0C87;
	LCD->RAM[3] &= 0xFFFFFFF0;
	LCD->RAM[4] &= 0x083D0C87;
	LCD->RAM[5] &= 0xFFFFFFF0;
	LCD->RAM[6] &= 0x083D0C87;
	LCD->RAM[7] &= 0xFFFFFFF0;
	LCD->SR |= LCD_SR_UDR;
}


void LCD_DisplayString(uint8_t* ptr){
	uint8_t LCD_pos, ptr_pos;
	bool point=0;
	bool colon=0;
	bool skip1=0;
	bool skip2=0;
	for(LCD_pos = 0, ptr_pos = 0; (ptr[ptr_pos] != 0x00)&&(LCD_pos<6); LCD_pos++, ptr_pos++){
		if(LCD_pos < 4){
			if(ptr[ptr_pos+1] == '.'){
				point=1;
				skip1=1;
			}
			else if(ptr[ptr_pos+1] == ':'){
				colon=1;
				skip2=1;
			}
		}
		LCD_WriteChar(&ptr[ptr_pos], point, colon, LCD_pos);
		if(skip1){skip1=0;ptr_pos++;point=0;}
		else if(skip2){skip2=0;ptr_pos++;colon=0;}
	}
}
  ...
  switch (position) {
    ...
    case 0:
      ...
      LCD->RAM[6] &= ~( 1U << 4 | 1U << 23 | 1U << 22 | 1U << 3 );
			/* 1G 1B 1M 1E */
      LCD->RAM[0] |= (uint32_t)(((digit[0] & 0x1) << 4) | (((digit[0] & 0x2) >> 1) << 23) | (((digit[0] & 0x4) >> 2) << 22) | (((digit[0] & 0x8) >> 3) << 3));
      /* 1F 1A 1C 1D  */
      LCD->RAM[2] |= (uint32_t)(((digit[1] & 0x1) << 4) | (((digit[1] & 0x2) >> 1) << 23) | (((digit[1] & 0x4) >> 2) << 22) | (((digit[1] & 0x8) >> 3) << 3));
      /* 1Q 1K 1Col 1P  */
      LCD->RAM[4] |= (uint32_t)(((digit[2] & 0x1) << 4) | (((digit[2] & 0x2) >> 1) << 23) | (((digit[2] & 0x4) >> 2) << 22) | (((digit[2] & 0x8) >> 3) << 3));
      /* 1H 1J 1DP 1N  */
      LCD->RAM[6] |= (uint32_t)(((digit[3] & 0x1) << 4) | (((digit[3] & 0x2) >> 1) << 23) | (((digit[3] & 0x4) >> 2) << 22) | (((digit[3] & 0x8) >> 3) << 3));
    ...
    case 1:
      ...
      LCD->RAM[6] &= ~( 1U << 6 | 1U << 13 | 1U << 12 | 1U << 5 );
			/* 2G 2B 2M 2E */
      LCD->RAM[0] |= (uint32_t)(((digit[0] & 0x1) << 6) | (((digit[0] & 0x2) >> 1) << 13) | (((digit[0] & 0x4) >> 2) << 12) | (((digit[0] & 0x8) >> 3) << 5));
      /* 2F 2A 2C 2D  */
      LCD->RAM[2] |= (uint32_t)(((digit[1] & 0x1) << 6) | (((digit[1] & 0x2) >> 1) << 13) | (((digit[1] & 0x4) >> 2) << 12) | (((digit[1] & 0x8) >> 3) << 5));
      /* 2Q 2K 2Col 2P  */
      LCD->RAM[4] |= (uint32_t)(((digit[2] & 0x1) << 6) | (((digit[2] & 0x2) >> 1) << 13) | (((digit[2] & 0x4) >> 2) << 12) | (((digit[2] & 0x8) >> 3) << 5));
      /* 2H 2J 2DP 2N  */
      LCD->RAM[6] |= (uint32_t)(((digit[3] & 0x1) << 6) | (((digit[3] & 0x2) >> 1) << 13) | (((digit[3] & 0x4) >> 2) << 12) | (((digit[3] & 0x8) >> 3) << 5));
    ...
    case 2:
      ...
      LCD->RAM[6] &= ~( 1U << 15 | 1U << 29 | 1U << 28 | 1U << 14 );
			/* 3G 3B 3M 3E */
      LCD->RAM[0] |= (uint32_t)(((digit[0] & 0x1) << 15) | (((digit[0] & 0x2) >> 1) << 29) | (((digit[0] & 0x4) >> 2) << 28) | (((digit[0] & 0x8) >> 3) << 14));
      /* 3F 3A 3C 3D */
      LCD->RAM[2] |= (uint32_t)(((digit[1] & 0x1) << 15) | (((digit[1] & 0x2) >> 1) << 29) | (((digit[1] & 0x4) >> 2) << 28) | (((digit[1] & 0x8) >> 3) << 14));
      /* 3Q 3K 3Col 3P  */
      LCD->RAM[4] |= (uint32_t)(((digit[2] & 0x1) << 15) | (((digit[2] & 0x2) >> 1) << 29) | (((digit[2] & 0x4) >> 2) << 28) | (((digit[2] & 0x8) >> 3) << 14));
      /* 3H 3J 3DP  3N  */
      LCD->RAM[6] |= (uint32_t)(((digit[3] & 0x1) << 15) | (((digit[3] & 0x2) >> 1) << 29) | (((digit[3] & 0x4) >> 2) << 28) | (((digit[3] & 0x8) >> 3) << 14));
    ...
    case 3:
      ...
      LCD->RAM[6] &= ~( 1U << 31 | 1U << 30);
			LCD->RAM[7] &= ~( 1U << 1 | 1U << 0 );
			/* 4G 4B 4M 4E */
      LCD->RAM[0] |= (uint32_t)(((digit[0] & 0x1) << 31) | (((digit[0] & 0x8) >> 3) << 30));
			LCD->RAM[1] |= (uint32_t)((((digit[0] & 0x2) >> 1) << 1) | (((digit[0] & 0x4) >> 2) << 0));
      /* 4F 4A 4C 4D */
      LCD->RAM[2] |= (uint32_t)(((digit[1] & 0x1) << 31) | (((digit[1] & 0x8) >> 3) << 30));
			LCD->RAM[3] |= (uint32_t)((((digit[1] & 0x2) >> 1) << 1) | (((digit[1] & 0x4) >> 2) << 0));
      /* 4Q 4K 4Col 4P  */
      LCD->RAM[4] |= (uint32_t)(((digit[2] & 0x1) << 31) | (((digit[2] & 0x8) >> 3) << 30));
			LCD->RAM[5] |= (uint32_t)((((digit[2] & 0x2) >> 1) << 1) | (((digit[2] & 0x4) >> 2) << 0));
      /* 4H 4J 4DP  4N  */
      LCD->RAM[6] |= (uint32_t)(((digit[3] & 0x1) << 31) | (((digit[3] & 0x8) >> 3) << 30));
			LCD->RAM[7] |= (uint32_t)((((digit[3] & 0x2) >> 1) << 1) | (((digit[3] & 0x4) >> 2) << 0));
    ...
    case 4:
      ...
      LCD->RAM[6] &= ~( 1U << 25 | 1U << 24 );
			LCD->RAM[7] &= ~( 1U << 3 | 1U << 2 );
			/* 5G 5B 5M 5E */
      LCD->RAM[0] |= (uint32_t)((((digit[0] & 0x2) >> 1) << 25) | (((digit[0] & 0x4) >> 2) << 24));
			LCD->RAM[1] |= (uint32_t)(((digit[0] & 0x1) << 3) | (((digit[0] & 0x8) >> 3) << 2));
      /* 5F 5A 5C 5D */
      LCD->RAM[2] |= (uint32_t)((((digit[1] & 0x2) >> 1) << 25) | (((digit[1] & 0x4) >> 2) << 24));
			LCD->RAM[3] |= (uint32_t)(((digit[1] & 0x1) << 3) | (((digit[1] & 0x8) >> 3) << 2));
      /* 5Q 5K 5Col 5P  */
      LCD->RAM[4] |= (uint32_t)((((digit[2] & 0x2) >> 1) << 25) | (((digit[2] & 0x4) >> 2) << 24));
			LCD->RAM[5] |= (uint32_t)(((digit[2] & 0x1) << 3) | (((digit[2] & 0x8) >> 3) << 2));
      /* 5H 5J 5DP  5N  */
      LCD->RAM[6] |= (uint32_t)((((digit[3] & 0x2) >> 1) << 25) | (((digit[3] & 0x4) >> 2) << 24));
			LCD->RAM[7] |= (uint32_t)(((digit[3] & 0x1) << 3) | (((digit[3] & 0x8) >> 3) << 2));
    ...
    case 5:
      ...
      LCD->RAM[6] &= ~( 1U << 17 | 1U << 8 | 1U << 9 | 1U << 26 );
			/* 6G 6B 6M 6E */
      LCD->RAM[0] |= (uint32_t)(((digit[0] & 0x1) << 17) | (((digit[0] & 0x2) >> 1) << 8) | (((digit[0] & 0x4) >> 2) << 9) | (((digit[0] & 0x8) >> 3) << 26));
      /* 6F 6A 6C 6D */
      LCD->RAM[2] |= (uint32_t)(((digit[1] & 0x1) << 17) | (((digit[1] & 0x2) >> 1) << 8) | (((digit[1] & 0x4) >> 2) << 9) | (((digit[1] & 0x8) >> 3) << 26));
      /* 6Q 6K 6Col 6P  */
      LCD->RAM[4] |= (uint32_t)(((digit[2] & 0x1) << 17) | (((digit[2] & 0x2) >> 1) << 8) | (((digit[2] & 0x4) >> 2) << 9) | (((digit[2] & 0x8) >> 3) << 26));
      /* 6H 6J 6DP  6N  */
      LCD->RAM[6] |= (uint32_t)(((digit[3] & 0x1) << 17) | (((digit[3] & 0x2) >> 1) << 8) | (((digit[3] & 0x4) >> 2) << 9) | (((digit[3] & 0x8) >> 3) << 26));
